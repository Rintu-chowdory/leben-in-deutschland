...
var moduleProgress = mysqlTable("moduleProgress", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull(),
  moduleName: varchar("moduleName", { length: 100 }).notNull(),
  // anmeldung, bank, health, visa, integration, tax
  completionPercentage: int("completionPercentage").default(0).notNull(),
  checklistItems: json("checklistItems").$type().default([]),
  savedResources: json("savedResources").$type().default([]),
...
var anmeldungOffices = mysqlTable("anmeldungOffices", {
  id: int("id").autoincrement().primaryKey(),
...
var bankRecommendations = mysqlTable("bankRecommendations", {
  id: int("id").autoincrement().primaryKey(),
  name: varchar("name", { length: 255 }).notNull(),
...
var healthInsuranceProviders = mysqlTable("healthInsuranceProviders", {
  id: int("id").autoincrement().primaryKey(),
  name: varchar("name", { length: 255 }).notNull(),
  type: mysqlEnum("type", ["gkv", "pkv"]).notNull(),
...
var integrationCourses = mysqlTable("integrationCourses", {
  id: int("id").autoincrement().primaryKey(),
...
var visaPermitTypes = mysqlTable("visaPermitTypes", {
  id: int("id").autoincrement().primaryKey(),
  name: varchar("name", { length: 255 }).notNull(),
  code: varchar("code", { length: 50 }).notNull().unique(),
  description: text("description"),
  eligibilityCriteria: json("eligibilityCriteria").$type().default([]),
  requiredDocuments: json("requiredDocuments").$type().default([]),
  processingTime: varchar("processingTime", { length: 100 }),
  validityPeriod: varchar("validityPeriod", { length: 100 }),
  applicationSteps: json("applicationSteps").$type().default([]),
  createdAt: timestamp("createdAt").defaultNow().notNull()
});
...
async function getOrCreateModuleProgress(userId, moduleName) {
  const db = await getDb();
  if (!db) return null;
...
async function updateModuleProgress(userId, moduleName, data) {
  const db = await getDb();
  if (!db) return null;
...
async function getUserModuleProgress(userId) {
  const db = await getDb();
  if (!db) return [];
  return await db.select().from(moduleProgress).where(eq(moduleProgress.userId, userId));
}
async function getAnmeldungOffices(city) {
  const db = await getDb();
  if (!db) return [];
  if (city) {
...
async function getHealthInsuranceProviders(type) {
  const db = await getDb();
  if (!db) return [];
  if (type) {
...
async function getIntegrationCourses(filters) {
  const db = await getDb();
  if (!db) return [];
  const conditions = [];
  if (filters?.city) {
  ...
  if (filters?.courseType) {
    conditions.push(eq(integrationCourses.courseType, filters.courseType));
  }
...
var appRouter = router({
  ...
  subscription: router({
    ...
    update: protectedProcedure.input(z2.object({
      tier: z2.enum(["free", "premium"]).optional(),
      stripeCustomerId: z2.string().optional(),
      stripeSubscriptionId: z2.string().optional(),
      status: z2.enum(["active", "inactive", "canceled"]).optional(),
      renewalDate: z2.date().optional()
    })).mutation(async ({ ctx, input }) => {
      return await updateSubscription(ctx.user.id, input);
    })
  }),
  // ============ MODULE PROGRESS ============
  moduleProgress: router({
    get: protectedProcedure.input(z2.object({
      moduleName: z2.string()
    })).query(async ({ ctx, input }) => {
      return await getOrCreateModuleProgress(ctx.user.id, input.moduleName);
    }),
    getAll: protectedProcedure.query(async ({ ctx }) => {
      return await getUserModuleProgress(ctx.user.id);
    }),
    update: protectedProcedure.input(z2.object({
      moduleName: z2.string(),
      completionPercentage: z2.number().optional(),
      checklistItems: z2.array(z2.object({
        id: z2.string(),
        label: z2.string(),
        completed: z2.boolean()
      })).optional(),
      savedResources: z2.array(z2.object({
        id: z2.string(),
        title: z2.string(),
        url: z2.string()
      })).optional(),
      personalNotes: z2.string().optional()
    })).mutation(async ({ ctx, input }) => {
      const { moduleName, ...data } = input;
      return await updateModuleProgress(ctx.user.id, moduleName, data);
    })
  }),
  // ============ ANMELDUNG ============
  anmeldung: router({
    getOffices: publicProcedure.input(z2.object({
      city: z2.string().optional()
    })).query(async ({ input }) => {
      return await getAnmeldungOffices(input.city);
    }),
  ...
  banks: router({
    getRecommendations: publicProcedure.query(async () => {
      return await getBankRecommendations();
    })
  }),
  // ============ HEALTH INSURANCE ============
  healthInsurance: router({
    getProviders: publicProcedure.input(z2.object({
      type: z2.enum(["gkv", "pkv"]).optional()
    })).query(async ({ input }) => {
      return await getHealthInsuranceProviders(input.type);
    })
  }),
  // ============ INTEGRATION COURSES ============
  integrationCourses: router({
    getCourses: publicProcedure.input(z2.object({
      city: z2.string().optional(),
      courseType: z2.string().optional()
    })).query(async ({ input }) => {
      return await getIntegrationCourses({
        city: input.city,
        courseType: input.courseType
      });
    }),
    getCities: publicProcedure.query(async () => {
      return await getCourseCities();
    })
  }),
  // ============ VISA & PERMITS ============
  visa: router({
    getPermitTypes: publicProcedure.query(async () => {
      return await getVisaPermitTypes();
    }),
    getPermitByCode: publicProcedure.input(z2.object({
      code: z2.string()
    })).query(async ({ input }) => {
      return await getVisaPermitByCode(input.code);
    })
  }),
  // ============ STRIPE CHECKOUT ============
  checkout: router({
    createSession: protectedProcedure.input(z2.object({
      tier: z2.enum(["premium"])
    })).mutation(async ({ ctx, input }) => {
      return {
